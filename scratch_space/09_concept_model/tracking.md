...
| 5.2 | Real-run perf follow-ups      | [`05.2_perf_followups.md`](05.2_perf_followups.md)       | superseded | Non-blocking observations from the successful en/pt run, kept as the record of what was seen; the fixes happened in the plans it routed to. Slug collisions halved deterministically by 5.55 (tier-2 -> phase 8); the >5 min load and the memory ceiling were root-caused and **fixed** in 5.3 (swap thrash from pydantic double-materialization, not `get_all_*`: cold load 16 s, 593 MB), and the five-language build then ran in 5.56 without the deferred restructure. Nothing open. |
...
| 5.4 | Preliminary data quality checks | [`05.4_data_quality.md`](05.4_data_quality.md)         | done   | Read-only quality pass over the first build: count/emptiness/cross-lingual-balance/trust checks as bounded DuckDB queries; diagnose the `house` "definition = lemma" defect (sparse OMW glosses + sense-blind kaikki join); full OMW/kaikki metadata catalog (kept/dropped/promote); other datasets + licensing. Routes findings to phases 6/7/8/10. Both halves landed: the measurement ran, and the "these become a regression harness" note became `lexicon/quality.py` + the auto-generated `report.md` via 5.56, which has gated every corpus change since. The must-fix reconciliation class reads 0; `def==lemma` reached 0 (5.55) and is now rebuild-durable (5.58); orphans and POS measured clean. Frequency/CEFR (6), remaining relations (7) and licensing (10) stay open by design. |
| 5.5 | Cleanup: re-cut around OMW backbone | [`05.5_cleanup.md`](05.5_cleanup.md)            | done | Execute 5.4's drop-kaikki decision: hard-delete the kaikki enrichment path, anchor on the concept/gloss/sense triple (OMW + CILI English fallback), one isolated loader per dataset, promote permissive OMW fields (examples/lexfile/`tag_count`/relations) for phases 6/7, LLM cleanup pass, regression-gated rebuild with no CC-BY-SA. Reopens phase 5's enrichment leg. **Steps 1-4 and 6-7 done** (kaikki removed; CILI English fallback; one isolated loader per dataset; Step-4 field promotion; rebuild + gate + license snapshot via 05.56); **All seven steps done**: Step 5 closed by 5.55 on 2026-09-01 (slug tier 1, def==lemma re-scope + repair to 0), Steps 1-4 and 6-7 earlier. Two deliberate deferrals carried to phase 8 / on-demand: LLM slug tier-2 qualifiers and non-en gloss backfill. |
...
- 2026-09-01 : ran 5.55's maintenance loop on the real corpus and **closed 5.55**. The
  single thin gloss (`c__mind-noun-cognition__93923c10626c`, it "attenzione") was repaired
  through the real propose -> review -> apply -> gate path; `definition == lemma` 1 -> 0,
  `DEFINITION_EQUALS_LEMMA_BASELINE` lowered 1 -> 0 so any newly introduced thin gloss now
  fails the gate. Exactly one concepts row changed (full-row anti-join against a pre-apply
  copy: same count, same schema, en gloss + examples intact), re-tagged `omw` -> `llm`;
  the table is now 117,658 `omw` + 1 `llm`. All four invariants PASS; `report.md`
  regenerated, and `slug_collisions_top` gained a `, slug` tiebreak so regeneration is
  byte-stable instead of reshuffling ties on every run.
  **Caveat**: no LLM API key in `~/cred/lang-tools/.env`, and the call was skipped by
  choice rather than copying a key between cred files - the reviewer authored the gloss,
  so `llm/gloss_repair.py` + `v1.jinja` remain tested but never run against a live model
  ...
  **Phase 5.5 closed** in the same pass: every "Done when" bullet holds (no kaikki rows,
  en gloss ~100%, def==lemma 0, dangling edges / lemmas-without-sense 0, OMW fields
...
- 2026-09-01 : **phase 5.58** - made curated gloss repairs survive a rebuild, closing the
  gap 5.57 exposed. Accepted `GlossProposal` rows now live in
  `data/curated/gloss_overrides.jsonl`, committed to git (force-added, like the
  `data/bootstrap/*.jsonl` seed, since `data/` is ignored wholesale), and `build_initial`
  applies them between `transform` and the write via `load_gloss_overrides` /
  `apply_gloss_overrides` - same model and same file format as the review artifact, so the
  maintenance loop's output *is* the build's input rather than a second schema. Applied
  rows are re-tagged `llm` exactly as the Parquet path does; the applied/stale counts reach
  `BuildSummary` and `_build.json`, so a build's curated state is auditable from the
  manifest. A stale override (an id the build no longer has, e.g. after a re-slug) warns
  and is counted, never fatal: a hard failure would block every rebuild after a legitimate
  re-slug, and the gate already catches the consequence.
  Proof: rebuilt from scratch with **no post-hoc apply** - "Applied 1 curated gloss
  override(s), 0 stale", the it gloss came back, the row is tagged `llm`,
  `definition == lemma` 0, four invariants PASS. The rebuild that reverted the repair in
  5.57 now reproduces it. `apply_gloss_proposals` stays: it is still how you fix the corpus
...
- 2026-09-01 : **phase 5.59** - regenerated the committed sample seed from the real build
  ...
  pyright green.
- 2026-09-01 : closed the two stale `draft` rows whose work had landed elsewhere, so the
  index stops implying open work. **5.2 -> superseded**: it was always the record of the
  first successful en/pt run, and all three observations are closed - slug collisions
  halved by 5.55 (tier-2 carried to phase 8), and the >5 min load and borderline memory
  root-caused and fixed in 5.3 (swap thrash from pydantic double-materialization, not the
  suspected `get_all_*` scan; cold load 16 s / 593 MB), after which 5.56's five-language
  build ran at ~1.5 GB without the deferred per-language restructure. **5.4 -> done**: a
  measure-and-route plan whose measurement ran and whose "these checks become a lightweight
  regression harness" note became `lexicon/quality.py` + the auto-generated `report.md`
  (5.56); its must-fix reconciliation class reads 0, and its routed findings are either
