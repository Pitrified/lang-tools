...
app (health, CORS, security headers, error handling) plus the word read router.
...
def build_app() -> FastAPI:
    ...
    Returns:
        Configured FastAPI application instance serving the word read API.
    """
    config = get_webapp_params().to_config()

    return create_app(
        config=config,
        extra_routers=[words_router],
    )
