...
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/lang_tools/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/lang_tools/` – Main application code
  - `src/lang_tools/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/lang_tools/.env`.
- Secrets should not be committed.

## Related

- `src/lang_tools/config/lang_tools_paths.py`
